...
from online_customer o inner join address a using (Address_id) 
inner join order_header using (customer_id) 
where Order_status= 'Cancelled'; 


#Q5
select * from shipper; select * from address; 
select * from ORDER_ITEMS; select Bdr.Shipper_Name, Bdr.City, Count(Distinct Adr.CUSTOMER_ID) Customer_Count 
from order_header Adr, (select Adr.Shipper_Name, Adr.SHIPPER_ID, Bdr.City, Bdr.ADDRESS_ID 
from SHIPPER Adr, ADDRESS Bdr where Adr.SHIPPER_ADDRESS = Bdr.ADDRESS_ID and Adr.SHIPPER_NAME = 'DHL') Bdr 
where Adr.SHIPPER_ID = Bdr.SHIPPER_ID 
Group by Bdr.SHIPPER_NAME,Bdr.City ;


#Q6
select ca.CUSTOMER_ID, concat(CUSTOMER_FNAME,' ',CUSTOMER_LNAME) as Full_Name, 
sum(ol.PRODUCT_QUANTITY) as total_quantity, sum(ol.PRODUCT_QUANTITY*pro.PRODUCT_PRICE) as total_value 
from online_customer ca 
join order_header ohl on ca.CUSTOMER_ID=ohl.CUSTOMER_ID 
join order_items ol on ohl.ORDER_ID=ol.ORDER_ID join product pro on ol.PRODUCT_ID=pro.PRODUCT_ID 
...
CARTON_VOLUME from carton car where CARTON_ID=10) 
...
from product pro join order_items ol on pro.PRODUCT_ID=ol.PRODUCT_ID 
...
where ol.ORDER_ID in (select ol.ORDER_ID from order_items 
where PRODUCT_ID='201' and ohl.ORDER_STATUS='shipped' and ad.CITY not in ('Bangalore','New Delhi') ) 
group by pro.PRODUCT_ID,pro.PRODUCT_DESC 
order by total_quantity desc;

#Q10
select ohl.ORDER_ID, ocl.CUSTOMER_ID, concat(CUSTOMER_FNAME,' ',CUSTOMER_LNAME) as Name , 
sum(ol.PRODUCT_QUANTITY) as Total_quantity from online_customer ocl 
join order_header ohl on ocl.CUSTOMER_ID=ohl.CUSTOMER_ID 
join order_items ol on ohl.ORDER_ID=ol.ORDER_ID join address a on ocl.ADDRESS_ID=a.ADDRESS_ID 
where ol.ORDER_ID %2= 0 and a.PINCODE not like '5%' group by 1,2;
